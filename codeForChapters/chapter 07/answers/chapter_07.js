...
const addVATcurried = (rate) => (amount) =>
  amount * (1 + rate / 100);
...
const f1 = curry(make3); // f1 is a function, that will fix make3's 1st parameter
...
const curry2 = (fn, len = fn.length) =>
  len === 0
    ? fn()
    : (p) => curry2(fn.bind(null, p), len - 1);

const sum2 = (...args) => args.reduce((x, y) => x + y, 0);
console.log(sum2.length); // 0; curry() wouldn't work

console.log(sum2(1, 5, 3)); // 9
console.log(sum2(1, 5, 3, 7)); // 16
console.log(sum2(1, 5, 3, 7, 4)); // 20

curriedSum5 = curry2(sum2, 5); // curriedSum5 will expect 5 parameters
...
const partialCurryingByBind = (fn) =>
  fn.length === 0
    ? fn()
    : (...pp) =>
        partialCurryingByBind(fn.bind(null, ...pp));
...
const partialCurryByClosure = (fn) => {
  const curryize =
    ...
    (...args2) => {
      const allParams = [...args1, ...args2];
      return (
        allParams.length < func.length ? curryize : fn
      )(...allParams);
    };
  return curryize();
};
...
const partialCurryByClosure2 = (fn, len = fn.length) => {
  const curryize =
    ...
    (...args2) => {
      const allParams = [...args1, ...args2];
      return (allParams.length < len ? curryize : fn)(
        ...allParams
      );
    };
  return curried();
